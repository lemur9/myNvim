...
| 按键 | 模式 | 说明 |
|------|------|------|
| `<leader>e` | n | 浮动文件树（根目录） |
| `<leader>E` | n | 浮动文件树（当前目录） |
| `<C-e>` | n | 编辑器 / 文件树焦点切换 |
| `<leader>ge` | n | Git 文件树 |
| `<leader>be` | n | 缓冲区列表 |
...
这些是 Neovim 默认的 `g` 前缀快捷键；本配置已在 which-key 中补充中文说明。

| 按键 | 说明 |
|------|------|
| `gg` / `G` | 文件开头 / 文件末尾 |
| `gd` / `gD` | 跳转到定义 / 全局声明（本配置中 `gd` 会优先走 fzf-lua/LSP） |
| `gr` / `gI` / `gy` | 引用 / 实现 / 类型定义（本配置中由 fzf-lua/LSP 提供） |
| `gi` | 回到上次插入位置并进入插入模式 |
| `gv` | 重新选择上次可视区域 |
| `gn` / `gN` | 选择下一个 / 上一个搜索匹配 |
| `gj` / `gk` | 按屏幕行向下 / 向上移动 |
| `g0` / `g$` / `gm` | 到屏幕行行首 / 行尾 / 中间 |
| `ge` / `gE` | 向后到词尾 / WORD 词尾 |
| `gt` / `gT` | 下一个 / 上一个标签页 |
| `gu{motion}` / `gU{motion}` / `g~{motion}` | 转小写 / 转大写 / 切换大小写 |
| `gq{motion}` / `gw{motion}` | 格式化文本 / 格式化文本并保持光标 |
| `gJ` | 连接行，不插入空格 |
| `gp` | 粘贴并把光标放到粘贴内容之后 |
| `ga` / `g8` | 显示光标字符编码 / UTF-8 字节 |
| `gx` | 打开光标下链接或文件 |
| `g?{motion}` / `g??` | ROT13 编码操作符 / 当前行 ROT13 |
| `g&` | 对所有行重复上次替换 |
| `g'` / ``g` `` | 跳转到标记位置（不修改跳转列表） |
| `g+` / `g-` | 更新 / 更早的文本状态 |
| `g,` | 更新列表中的上一个位置 |

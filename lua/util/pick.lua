...
function M.open(command, opts)
  if not M.picker then
    return LemurVim.error("LemurVim.pick: picker not set")
  end
  ...
  if type(opts.cwd) == "boolean" then
    LemurVim.warn("LemurVim.pick: opts.cwd should be a string or nil")
    opts.cwd = nil
  end

  if not opts.cwd and opts.root ~= false then
    opts.cwd = LemurVim.root({ buf = opts.buf })
  end
...
function M.wrap(command, opts)
  opts = opts or {}
  return function()
    LemurVim.pick.open(command, vim.deepcopy(opts))
  end
end
